...
class ImageShowWidget(QWidget):
    ...
    def load_config(self):
        self.eyetracker_frequency = global_config.get_value(
            "eyetracker", "frequency", 60
        )
        self.image_show_time = (
            global_config.get_value("image_show", "last_time", 10) * 1000
        )
        self.image_show_interval = (
            global_config.get_value("image_show", "time_interval", 3) * 1000
        )
    ...
    def save_eye_data(self, filetype=("txt", "json")):
        ...
        if "json" in filetype:
            ...
            json.dump(
                self.eyetracker_wrap.get_gaze_data(), fid, indent=4, ensure_ascii=False
            )
            fid.close()
            ...
            if self.state & (1 << self.DisplayState.Ready):
                if self.state & (1 << self.DisplayState.Image):
                    ...
                    if self.cur_image_index < self.image_num - 1:
                        self.cur_image_index = self.cur_image_index + 1
                        self.cur_image_name = self.image_list[
                            self.cur_image_index
                        ].split(".")[0]
                        self.cur_image_file = os.path.join(
                            self.dir_imgdb, self.image_list[self.cur_image_index]
                        )
                        self.cur_image.load(self.cur_image_file)
                        self.cur_pixmap = QPixmap.fromImage(self.cur_image)
                        self.image_display.setPixmap(self.cur_pixmap)
                        self.current_eye_data_file = os.path.join(
                            self.dir_out_data, self.participant_id, self.cur_image_name
                        )
    ...
    def do_error_detection(self):
        ...
        if len(current_gaze_data) == 0 or (
            ...
            self.eye_detect_error_count = 0
        if self.eye_detect_error_count >= int(
            (self.image_show_time / self.detect_error_interval_ms) * 0.75
        ):
            self.detect_error_timer.stop()
            self.imgshow_timer.stop()
            if self.eyetracker_wrap.eyetracker:
